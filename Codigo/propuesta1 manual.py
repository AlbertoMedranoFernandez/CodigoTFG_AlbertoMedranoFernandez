    ...
    while True:
        ...
        if resultados.multi_face_landmarks:  # Si se detecta alguna cara
            for rostros in resultados.multi_face_landmarks:  # Mostrar cara detectada
                mp.solutions.drawing_utils.draw_landmarks(frame, rostros, mp.solutions.face_mesh.FACEMESH_CONTOURS,
                                                          None, drawingSpec)
                ...
                # Calibrado manual
                if calibrado == False:
                    cv2.putText(frame, f'Calibracion', (75, 60), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 3)
                    if len(puntos) < 2:
                        cv2.putText(frame, f'IMPORTANTE: Mantenga los ojos ABIERTOS.',
                                    (75, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                        cv2.putText(frame, f'Pulsa la tecla A dos veces para calibrar: {int(len(puntos))}', (75, 160),
